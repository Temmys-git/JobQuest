...
const Pagignate =({paginationProps})=> {
    const {totalPage,jobs,firstIndex,lastIndex,setCurrentPage,currentPage,setIsSearch,setSearch} = paginationProps
    ...
    return (
        ...
        <p>
<span>{firstIndex} </span> to <span>{lastIndex > jobs?.length ? jobs?.length:lastIndex }</span> of <span>{jobs?.length}</span> </p>
